...
synopsis2 = "detailed description:\n\
 1. Input files and parameters\n\
  ...
  - <BestHitPairs> is a tab-delimited file with protein-coding (pc) loci from \n\
     the query and the subject speices, respectively, pairing homologous subject\n\
 ...
 2. CL_type (co-linearity type) is decided as the following:\n\
  ...
  - 1)CL_type='lineage-spcific(ls)' if no subject locus was paried with the\n\
 ...
 4. Misc. details:\n\
  ...
  - for 'cl' loci, numbers of colinear loci within the window ('cl_counts') are\n\
     reported, e.g. 'cl_u3d3' means '>=6 colinear loci detected, with >=3 to\n\
     the upstream and >=3 to the downstream'.\n\
 ...
 by ohdongha@gmail.com 20170316 ver 0.5.2\n"
...
#20160309 ver 0.3 #now it can accept the 3 parameters as arguements.
...
sp2_OGid_dict = dict() # dict with key = geneID, value = OGid, for the subject species
...
for line in args.File4Qury:
	if header == 1:
		sp1_code = line.split('\t')[11].replace('_OG','').strip()
		header = 0
	else:	
		try:
			tok = line.split('\t')
...
for line in args.File4Sbjt:
	if header == 1:
		sp2_code = line.split('\t')[11].replace('_OG','').strip()
		header = 0
	else:	
		try:
			tok = line.split('\t')
...
#							if sp1_bestHit_in_sp2_dict[geneID_being_searched] in sp2_locusID_dict : ## removed in ver 1.31
							if sp1_bestHit_in_sp2_dict[geneID_being_searched] in sp2_locusID_dict \
									and bestHit_locus_prev != sp2_locusID_dict[sp1_bestHit_in_sp2_dict[geneID_being_searched]]:	## added in ver 1.31 
...
for line in args.File4Qury:
	...
	if header == 1:
		args.Outfile.write(line.strip() + '\t' \
					...
					+ sp2_code + "_geneID" + '\t' + sp2_code + "_OG" + '\t' + sp2_code + "locusID" + '\t' + sp2_code + "TDid" +'\n')
		header = 0
		line_written = 1
	else:
		try:
			...
			if geneID in sp1_bestHit_in_sp2_dict:
				...
				if bestHit in sp2_locusID_dict:
					args.Outfile.write(line.strip() + '\t' + sp1_CLtype_dict[geneID] + '\t' + bestHit + '\t' + sp2_OGid_dict[bestHit] + '\t' + str(sp2_locusID_dict[bestHit]) + '\t' + sp2_TDid_dict[bestHit] +'\n')
					line_written = 1
				else:
