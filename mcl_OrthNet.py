...
synopsis2 = "detailed description:\n\
 ...
 1. Input:\n\
  - expects './<Project>.clstrd.edges' generated by 'create_OrthNet.py',\n\
     i.e. clstrID, quryID, sbjtID, CL_type(q->s), and CL_type(s->q),\n\
     tab-delimited, one edge per line, sorted by clstrID\n\
 ...
 2. Clustering process and parameters:\n\
  ...
  - other parameters for mcl:\n\
     '-I'|'--graininess': 'graininess' of mcl; a value between 1.2 (rougher\n\
      clusters) and 5 (finer clusters); default=2\n\
 ...
 3. Output:\n\
  - parsed mcl results are combined and written to, if default parameters used:\n\
     '<Project>_TD1.5_rC1.2_rNC0.5_uC0.6_uNC0.25_I2.0_mclOut.PC.txt', \n\
     ('PC' == 'Parsed and Concatanated').\n\
  - '-c'|'--clean_tempFiles': remove all temp files.\n\n\
 by ohdongha@gmail.com ver0.2 20160918\n"
...
## option to receive PATH to output files
parser.add_argument('-w', '--weightMatrix', dest="weightMatrix", default="weights4mcl_OrthNet.list", help="see below")
parser.add_argument('-o', '--Path2Output', dest="Path2Output", type=str, default=".\mcl", help="PATH for temp. and output files")
parser.add_argument('-c', '--clean_tempFiles', action="store_true", default=False, help="clean temp. files after mcl runs")
...
parser.add_argument('-I', '--graininess', dest="graininess", type=float, default=2, help="'graininess' of mcl; see below")
...
fileName_mclOutput_parsed_combined = path_output + args.Project + '_TD' + str(edgeWeights_dict['TD']) + '_rC' + str(edgeWeights_dict['CL_rc']) \
			+ '_rNC' + str(edgeWeights_dict['nCL_rc']) + '_uC' + str(edgeWeights_dict['CL']) + '_uNC' + str(edgeWeights_dict['nCL']) \
			+ '_I' + str(args.graininess) + '_mclOut.PC.txt'
num_lines = 0
...
#			+ " -o " + fileName_mclOutput, shell=True) # hard to make mcl complitely quiet
	subprocess.call("mcl " + fileName_mclInput + " --abc -I " + str(args.graininess) \
